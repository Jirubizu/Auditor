...
namespace Auditor.Services
{
    ...
    public class PaginatedMessage
    {
        ...
        public PaginatedMessage(IEnumerable<EmbedBuilder> builders, string title = "", Color? embedColor = null,
            IUser user = null, AppearanceOptions options = null)
        {
            List<Embed> embeds = new List<Embed>();
            int i = 1;

            var embedBuilders = builders as EmbedBuilder[] ?? builders.ToArray();
            foreach (EmbedBuilder builder in embedBuilders)
            {
                builder.Title ??= title;
                builder.Color ??= embedColor ?? Color.Default;
                builder.Footer ??= new EmbedFooterBuilder().WithText($"Page {i++}/{embedBuilders.Length}");
    ...
    public class PaginationService
    {
        ...
        public async Task<IUserMessage> SendMessageAsync(IMessageChannel channel, PaginatedMessage paginated)
        {
            ...
            {
                Task _ = Task.Delay(paginated.Options.Timeout).ContinueWith(async _ =>
                {
        ...
        private async Task OnReactionAdded(Cacheable<IUserMessage, ulong> messageParam, ISocketMessageChannel channel,
            SocketReaction reaction)
        {
            IUserMessage message = await messageParam.GetOrDownloadAsync();
